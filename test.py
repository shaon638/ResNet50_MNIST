...
def load_dataset() -> CUDAPrefetcher:
    test_dataset = ImageDataset("/ssd_scratch/cvit/shaon/mnist_test/test",
                                config.image_size,
...
def main() -> None:
    ...
    #print("****************")
    #str1 = config.model_weights_path
    #print(str1)
    #print(f"model path `{config.model_weights_path}` successfully.")
    
    resnet_model = build_model()
    print(f"Build `{config.model_arch_name}` model successfully.")

    # Load model weights
    resnet_model, _, _, _, _, _ = load_state_dict(resnet_model, "results/pretrained_models/best.pth.tar")
    ...
    count = 1
    image_dir = "/ssd_scratch/cvit/shaon/mnist_test/test"
